...
import com.opsmonitor.common.ApplicationContextHelper;
import com.opsmonitor.config.MailConfig;
...
public class WarnMailUtil {
    ...
    public static final String content_suffix = "<p><a target='_blank' OpsMonitor 运维监控系统";
    ...
    private static AlarmRecordService alarmRecordService = (AlarmRecordService) ApplicationContextHelper.getBean(AlarmRecordService.class);

    /**
     * 记录一条告警到告警中心（独立于邮件发送，邮件发送失败也会记录）
     */
    private static void recordAlarm(String alarmType, String hostname, String title, String content, String level) {
        try {
            if (alarmRecordService != null) {
                alarmRecordService.save(alarmType, hostname, title, content, level);
            }
        } catch (Exception e) {
            logger.error("记录告警中心数据异常：", e);
        }
    }
    ...
    public static boolean sendWarnInfo(MemState memState) {
        ...
        if (memState.getUsePer() != null && memState.getUsePer() >= mailConfig.getMemWarnVal()) {
            try {
                ...
                //记录告警中心
                recordAlarm("mem", memState.getHostname(), title, commContent, "warn");
                //发送邮件
    ...
    public static boolean sendCpuWarnInfo(CpuState cpuState) {
        ...
        if (cpuState.getSys() != null && cpuState.getSys() >= mailConfig.getCpuWarnVal()) {
            try {
                ...
                //记录告警中心
                recordAlarm("cpu", cpuState.getHostname(), title, commContent, "warn");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
    ...
    public static boolean sendDiskWarnInfo(DeskState deskState) {
        ...
        if (usePer >= mailConfig.getDiskWarnVal()) {
            try {
                ...
                //记录告警中心
                recordAlarm("disk", deskState.getHostname(), title, commContent, "warn");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
    ...
    public static boolean sendHeathInfo(HeathMonitor heathMonitor, boolean isDown) {
        ...
        if (isDown) {
            ...
            try {
                ...
                //记录告警中心
                recordAlarm("heath", heathMonitor.getHostIp(), title, commContent, "warn");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
            ...
            try {
                ...
                //记录告警中心（恢复通知也记录，方便追踪）
                recordAlarm("heath", heathMonitor.getHostIp(), title, commContent, "info");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
    ...
    public static boolean sendHostDown(SystemInfo systemInfo, boolean isDown) {
        ...
        if (isDown) {
            ...
            try {
                ...
                //记录告警中心
                recordAlarm("hostdown", systemInfo.getHostname(), title, commContent, "error");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
            ...
            try {
                ...
                String commContent = "主机已经恢复上线：" + systemInfo.getHostname() + "，备注：" + systemInfo.getRemark()
                        + "。";
                //记录告警中心（恢复也记录）
                recordAlarm("hostdown", systemInfo.getHostname(), title, commContent, "info");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
    ...
    public static boolean sendAppDown(AppInfo appInfo, boolean isDown) {
        ...
        if (isDown) {
            ...
            try {
                ...
                //记录告警中心
                recordAlarm("appdown", appInfo.getHostname(), title, commContent, "error");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
            ...
            try {
                ...
                //记录告警中心（恢复也记录）
                recordAlarm("appdown", appInfo.getHostname(), title, commContent, "info");
                //发送邮件
                sendMail(mailSet.getToMail(), title, commContent);
                //发送钉钉/企业微信
                WarnWebhookUtil.sendWarn(title, commContent);
    ...
    public static String sendMail(String mails, String mailTitle, String mailContent) {
        try {
            ...
            email.setSubject("[OpsMonitor] " + mailTitle);//标题
