...
import com.opsmonitor.dto.ChartInfo;
import com.opsmonitor.dto.NetIoStateDto;
...
public class DashboardController {
    ...
    public String systemInfoList(SystemInfo systemInfo, Model model, HttpServletRequest request) {
        ...
        try {
            ...
            // 全局搜索：hostname 模糊匹配
            String searchHost = request.getParameter("hostName");
            if (StringUtils.isEmpty(searchHost)) {
                searchHost = request.getParameter("hostname");
            }
            if (!StringUtils.isEmpty(searchHost)) {
                params.put("hostnameLike", searchHost.trim());
                url.append("&hostName=").append(searchHost.trim());
            // 主机分组筛选
            String groupName = request.getParameter("groupName");
            if (!StringUtils.isEmpty(groupName)) {
                params.put("groupName", groupName.trim());
                url.append("&groupName=").append(groupName.trim());
            // 主机分组列表（供筛选下拉）
            try {
                model.addAttribute("groupList", systemInfoService.selectGroups());
            } catch (Exception e) {
                model.addAttribute("groupList", new java.util.ArrayList<String>());
            }
            model.addAttribute("currentGroup", StringUtils.isEmpty(groupName) ? "" : groupName.trim());
            }
            }
            if (request.getParameter(StaticKeys.DASH_VIEW_ACCOUNT) != null) {
