...
import com.opsmonitor.util.TokenUtils;
        ...
        try {
            ...
            if (deskStateList != null) {
                for (Object jsonObjects : deskStateList) {
                    ...
                    // 按分区异步检测磁盘使用率告警，避免阻塞 Agent 数据上报响应
                    final DeskState deskState = bean;
                    Runnable runnable = () -> {
                        WarnMailUtil.sendDiskWarnInfo(deskState);
                    };
                    executor.execute(runnable);
                }
            }
