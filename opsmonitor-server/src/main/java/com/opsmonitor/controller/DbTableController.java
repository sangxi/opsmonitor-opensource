...
import com.opsmonitor.service.DbInfoService;
import com.opsmonitor.service.DbTableCountService;
import com.opsmonitor.service.DbTableService;
import com.opsmonitor.service.LogInfoService;
import com.opsmonitor.util.PageUtil;
import com.opsmonitor.util.jdbc.RDSConnection;
    ...
    public String saveDbTable(DbTable DbTable, Model model, HttpServletRequest request) {
        try {
            ...
            if (!StringUtils.isEmpty(whereVal)) {
                ...
                // 检查是否包含分号（可能用于执行多个SQL语句）
                if (whereVal.contains(";")) {
                    model.addAttribute("dbTable", DbTable);
                    List<DbInfo> dbInfoList = dbInfoService.selectAllByParams(new HashMap<>());
                    model.addAttribute("dbInfoList", dbInfoList);
                    model.addAttribute("msg", "where语句不能包含分号，请检查");
                    return "mysql/add";
                }
                // 检查是否包含注释符（可能用于注释掉部分SQL）
                if (whereVal.contains("--") || whereVal.contains("/*")) {
                    model.addAttribute("dbTable", DbTable);
                    List<DbInfo> dbInfoList = dbInfoService.selectAllByParams(new HashMap<>());
                    model.addAttribute("dbInfoList", dbInfoList);
                    model.addAttribute("msg", "where语句不能包含注释，请检查");
                    return "mysql/add";
                }
                DbTable.setWhereVal(whereVal);
            }
