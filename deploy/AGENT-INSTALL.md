...
base:
  ...
  # 通信 token，必须与 Server 端 application.yml 中的 params.omToken 一致
  omToken: opsmonitor
```
...
| `omToken` | 与 Server 端一致的通信 token，鉴权用 |
...
看到 `Started OpsMonitorServiceApplication` 即启动成功，按 Ctrl+C 停止。
...
```bash
cd opsmonitor-opensource
# 修改 .env 中的 SERVER_URL、BIND_IP、OMTOKEN
...
1. **查看 Agent 日志**：日志目录 `./log/opsmonitor-agent.log`，看到 `Started OpsMonitorServiceApplication` 即启动成功。
...
Agent 默认每分钟采集并上报一次。如需修改，编辑 `opsmonitor-agent/src/main/java/com/opsmonitor/ScheduledTask.java` 中的 `@Scheduled` 注解的 `fixedRate` 值（单位毫秒）后重新打包。
